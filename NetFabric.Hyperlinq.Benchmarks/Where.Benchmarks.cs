...
namespace NetFabric.Hyperlinq.Benchmarks
{
    ...
    public class WhereBenchmarks : BenchmarksBase
    {
        ...
        {
            var sum = 0;
            foreach (var item in ReadOnlyCollectionExtensions.AsValueEnumerable<TestCollection.Enumerable, TestCollection.Enumerable.Enumerator, int>(collectionValue, enumerable => enumerable.GetEnumerator()).Where(item => (item & 0x01) == 0))
                sum += item;
            return sum;
        }
