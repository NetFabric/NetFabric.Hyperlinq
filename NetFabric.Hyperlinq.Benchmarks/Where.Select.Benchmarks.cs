...
namespace NetFabric.Hyperlinq.Benchmarks
{
    ...
    public class WhereSelectBenchmarks : BenchmarksBase
    {
        ...
        { 
            var count = 0;
            foreach(var item in System.Linq.Enumerable.Select(System.Linq.Enumerable.Where(queue, _ => true), item => item))
                count++;
            return count;
        }
        ...
        { 
            var count = 0;
            foreach(var item in queue.Where(_ => true).Select(item => item))
                count++;
            return count;
        }
        ...
        { 
            var count = 0;
            foreach(var item in enumerableReference.Where(_ => true).Select(item => item))
                count++;
            return count;
        }
        ...
        { 
            var count = 0;
            foreach(var item in enumerableValue.Where(_ => true).Select(item => item))
                count++;
            return count;
        }
    }
