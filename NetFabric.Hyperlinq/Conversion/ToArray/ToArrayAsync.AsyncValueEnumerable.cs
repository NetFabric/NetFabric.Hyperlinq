...
namespace NetFabric.Hyperlinq
{
    public static partial class AsyncValueEnumerable
    {
        ...
        {
            var result = await ToArrayWithLengthAsync<TEnumerable, TEnumerator, TSource>(source, cancellationToken).ConfigureAwait(false);
            System.Array.Resize(ref result.Array, result.Length);
            return result.Array;
        }

        [Pure]
        static async ValueTask<ArrayWithLength<TSource>> ToArrayWithLengthAsync<TEnumerable, TEnumerator, TSource>(TEnumerable source, CancellationToken cancellationToken)
            where TEnumerable : IAsyncValueEnumerable<TSource, TEnumerator>
            where TEnumerator : struct, IAsyncEnumerator<TSource>
        {
