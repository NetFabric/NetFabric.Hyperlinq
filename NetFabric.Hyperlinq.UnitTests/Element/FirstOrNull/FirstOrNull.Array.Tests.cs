...
{
    public class FirstOrNullArrayTests
    {
        [Theory]
        [MemberData(nameof(TestData.SingleEmpty), MemberType = typeof(TestData))]
        public void FirstOrNull_With_Empty_Should_Return_Default(int[] source)
        {
            // Arrange

            // Act
            var result = Array.FirstOrNull<int>(source);
        ...
        public void FirstOrNullPredicate_With_Empty_Return_Default(int[] source, Func<int, long, bool> predicate)
        {
            ...
            // Act
            var result = Array.FirstOrNull<int>(source, predicate);
