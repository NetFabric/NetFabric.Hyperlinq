...
#pragma warning disable HLQ010 // Consider using a 'for' loop instead.
        ...
        {
            var sum = 0;
            foreach (var item in ListBindings.Select(source, item => new FatValueType(item.Value0 * 2)))
                sum += item.Value0;
            return sum;
        }
#pragma warning restore HLQ010 // Consider using a 'for' loop instead.
        ...
        {
            var items = ListBindings.Select(source, item => new FatValueType(item.Value0 * 2));
            var sum = 0;
            for (var index = 0; index < items.Count; index++)
                sum += items[index].Value0;
            return sum;
